...
def parse_time_from(log_file):
    ...
    size_pattern = r'Обработка матриц (\d+)x\d+'
    thread_pattern = r'Тестирование с (\d+) потока?ми?'
    time_pattern = r'Время выполнения \((\d+) потоков?\): (\d+) мс'
    ...
    for line in content.split('\n'):
        ...
        if time_match and current_size:
            time_ms = int(time_match.group(2))
            data.append({
                'size': current_size,
                'threads': int(time_match.group(1)),
                'time_sec': time_ms  # Конвертируем в секунды
            })
...
def plot_performance(df, output_file="performance_plot.png"):
    ...
    for i, threads in enumerate(thread_counts):
        ...
        plt.plot(thread_data['size'], thread_data['time_sec'],
                 ...
                 markersize=8, label=f'{threads} потоков')
