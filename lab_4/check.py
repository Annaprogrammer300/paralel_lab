...
def parse_time_from(log_file):
    """Извлекает время выполнения из файла stats.txt с новым форматом"""
    with open(log_file, 'r', encoding='utf-8') as f:
        content = f.read()

    # Регулярные выражения для нового формата
    pattern = r'Размер матрицы: (\d+)x\d+, Потоков: (\d+), Время: (\d+) мс'
    ...
    for line in content.split('\n'):
        ...
        if match:
            ...
            data.append({
                'size': size,
                'threads': threads,
                'time_sec': time_ms / 1000  # Конвертируем в секунды
            })

    return pd.DataFrame(data)


def plot_performance(df, output_file="grafic.png"):
    ...
    for i, threads in enumerate(thread_counts):
        ...
        plt.plot(thread_data['size'], thread_data['time_sec'], marker='*', color=colors[i], linewidth=2.5,
                 markersize=10, label=f'{threads} потоков')
...
def find_and_verify_all(base_dir="matrix_results"):
    ...
    parse_file = Path("matrix_results/stats.txt")
